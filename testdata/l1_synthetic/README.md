...
| 类别 | 条数 | 说明 |
|---|---|---|
| field(现场记录/讨论陈述) | 50 | 会议讨论、客户反馈、进度同步等陈述句,预期行为为记录整理(046~050 为路由验收补充,文本级无音频) |
| task_command(任务指令) | 50 | 完成/新建/提醒/延期/查询/取消/优先级等动作,expected 含动作与参数(046~050 同上) |
| experience(经验复盘口述) | 50 | 项目复盘、踩坑总结、方法论,预期行为为经验沉淀(046~050 同上) |
| interference(干扰与边界) | 25 | 同名歧义(8)、模糊表达、闲聊(8)、中英混说(9);歧义样本 expected 为 `{"action":"clarify"}`,闲聊为 `{"action":"none","behavior":"no_route"}` |

合计 175 条,全部带标准答案标注;其中 046~050 共 15 条为 FR-04 路由验收补足(三类各 50 条),仅文本(labels.jsonl/texts.jsonl),无音频,ASR 基准自动跳过。
