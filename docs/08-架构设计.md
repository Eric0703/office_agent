...
时序等价(2026-07-22):`intent.result`/`confirm.request` 可能经 WS 先于 HTTP 200 到达(如 duplicate 补推)——两种到达顺序收敛到同一终态;晚到的 `UploadDone` 在 Showing/ConfirmWait 无迁移,不得覆盖(A1-2 实测修复)。
...
| `skills.reminder` | 提醒调度:一次性定时提醒解析/创建/确认、卡片生命周期、到期触发 | `execute()` / `confirm_pending()` | FR-07 |
...
| `api` | HTTP/WS 端点装配、静态托管虚拟工牌;`/desk/*` 本机工作台接口(只读 + 完成/取消) | FastAPI app | FR-12 |
...
| 结果页 | 执行结果 | 成功 ≤3s 自动**返回原页面**(默认页或简报页);失败/未听清停留,点击(按键)返回,不一闪而过 |
