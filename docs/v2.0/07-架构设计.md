...
| `skills.field_note` | 现场记录:结构化笔记草稿、待确认队列、人工确认归档(本机 Markdown 落盘);归档时"## 待办"有效条目转任务草稿(task_drafts 表,只读展示,不进 tasks 表) | `process(record_id, transcript)` / `archive(draft_id)` / `extract_todo_titles(content_md)` | FR-05 |
...
| `skills.task_command` | 语音指令:参数抽取、歧义候选、幂等执行、多任务预览 | `execute(intent, record_id)` / `confirm_create(...)` / `cancel_pending(...)` / `complete_by_id(...)` | FR-08 |
...
| `api` | HTTP/WS 端点装配、静态托管虚拟工牌;`/desk/*` 本机工作台接口(只读 + 完成/取消/配对批准/草稿确认归档);核心流程装配:构建 `ProcessingDeps`、音频编排(取行/转写/清理)、统一投递 `_deliver` | FastAPI app | FR-12 |
...
| `task_drafts` | id TEXT PK;source_draft_id TEXT NOT NULL FK(来源笔记草稿);title TEXT NOT NULL;created_at TEXT NOT NULL;UNIQUE(source_draft_id, title) | 待办转任务草稿(FR-05 方案 D):笔记归档确认后抽取,只读展示,不进 tasks 表 |
